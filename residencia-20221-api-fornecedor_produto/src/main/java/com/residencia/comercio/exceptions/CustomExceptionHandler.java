...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
	 ...
	 * headers, HttpStatus status, WebRequest request) { List<FieldError>
	 * fieldErrors = ex.getBindingResult().getFieldErrors();
	 * 
	 * String fields =
	 * fieldErrors.stream().map(FieldError::getField).collect(Collectors.
	 * joining(", ")); String fieldsMessage =
	 * fieldErrors.stream().map(FieldError::getDefaultMessage).collect(Collectors.
	 * joining(", "));
	 * 
	 * HttpStatus httpStatus = HttpStatus.BAD_REQUEST; ErrorResponse error = new
	 * ErrorResponse(httpStatus.value(),
	 * "Falha na Validação dos Dados da Requisição", fields, fieldsMessage);
	 * 
	 * return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST); }
	 */
